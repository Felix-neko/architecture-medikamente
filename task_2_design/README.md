...
* `med_record_fields`: поля медкарты: поля, которые относятся к медкарте, их нужно просто не показыать сотрудникам регистратуры, а показывать только лечащему врачу (другим врачам тоже не показывать);  
